...
# KondoManager - Gestão de Condomínios
KondoManager é um software open source inovador para gestão de condomínios, desenvolvido em Laravel e base de dados MySQL, projetado para administradores de condomínio, mas também para condóminos.
...
- Gestão do quadro de avisos do condomínio
...
Para utilizadores menos experientes que desejam instalar o KondoManager num servidor próprio, criamos um assistente guiado para o processo de instalação.
...
4. Configure a base de dados MySQL no arquivo .env
```bash
DB_CONNECTION=mysql
DB_HOST=127.0.0.1
DB_PORT=3306
...
7. Preencha a base de dados com as configurações padrão
```bash
php  artisan  db:seed
