...
> **Stato:** Superato — verificato il 31/07/2026 su 1.10.0-beta.32, riverificato il 01/08/2026 su 1.10.0-beta.33
> Sostituito dalla voce [1.10.0-beta.9] di docs/changelog.md, che ha risolto il caso con una terza via non contemplata qui (l'eccedenza diventa strapagamento su una quota esistente, non una RataQuote nuova né una lettura del partitario), e dalla beta.10 / docs/credito_visibile_ovunque.md per la parte cross-gestione.
> Da non usare: l'avviso "non procedere con modifiche", le domande bloccanti §3, l'audit §4, le due opzioni §5 e i prossimi passi §6 sono tutti chiusi. Restano utili come riferimento le note strutturali §2.3 e §2.6 e le due query diagnostiche dell'Appendice A, ancora eseguibili così come sono.
<!-- /verifica-documentazione -->
...
<!-- rettifica -->
> ⚠️ **Non è più vero — verificato il 31/07/2026 su 1.10.0-beta.32.** Non è più così dalla beta.9: l'eccedenza finanziata da contanti aggiorna importo_pagato su una quota esistente, quindi ha un contenitore, compare in Nuovo Incasso come credito e si spende con "Usa credito".
> *Prova:* app/Actions/Gestionale/Movimenti/StoreIncassoRateAction.php:184-201; consumo del credito a :262-292 (credito_disponibile).
<!-- /rettifica -->
...
<!-- rettifica -->
> ⚠️ **Non è più vero — verificato il 31/07/2026 su 1.10.0-beta.32.** Il fix è stato deciso, implementato e rilasciato nella 1.10.0-beta.9, per una terza via non prevista dal documento: l'eccedenza viene agganciata come strapagamento all'ultima quota del pagante toccata dall'incasso (importo_pagato > importo), quindi diventa credito riutilizzabile senza creare una nuova RataQuote né leggere il partitario. La sola scrittura su anticipi_condomini è rimasta come fallback per il caso in cui non esista una quota di appoggio.
> *Prova:* app/Actions/Gestionale/Movimenti/StoreIncassoRateAction.php:184-201 (ramo eccedenzaCassaCents > 0, 'importo_pagato' => $eccedenzaCassaCents) e :218-223 (fallback: «l'eccedenza resta una pura scrittura contabile sugli anticipi»); accessor in app/Models/Gestionale/RataQuote.php:95. Voce di changelog: docs/changelog.md, sezione [1.10.0-beta.9] — «L'eccedenza viene ora agganciata come strapagamento all'ultima quota del pagante toccata dall'incasso».
<!-- /rettifica -->
  ```php
...
<!-- rettifica -->
> ⚠️ **Il filtro qui sopra fa perdere il caso più frequente — verificato il 01/08/2026 su 1.10.0-beta.33.** L'intestazione di questo documento dichiara le query dell'Appendice A «ancora eseguibili così come sono», e lo sono: è l'**istruzione di lettura** a essere incompleta. La riga «Anticipo / Eccedenza» viene scritta su **due conti diversi**, a seconda del ramo: se esiste una quota su cui accreditare lo strapagamento va sul conto con ruolo `crediti_condomini`; solo nel caso residuale in cui non ci sia nessuna quota va su `anticipi_condomini`. Filtrando per `anticipi_condomini` si trova solo il secondo caso — e si conclude «eccedenza non registrata» quando invece c'è.
> **Come leggerla davvero:** cercare per `note = 'Anticipo / Eccedenza'` e basta, poi guardare quale `conto_ruolo` è uscito: `crediti_condomini` = agganciata a una quota (via `rs.rata_id`); `anticipi_condomini` = eccedenza pura, senza quota di destinazione.
> *Prova:* app/Actions/Gestionale/Movimenti/StoreIncassoRateAction.php:199-226 — il ramo `if ($quotaAccredito)` scrive su `$contoCrediti->id` (:206), il ramo `else` su `$contoAnticipi->id` (:220); entrambi con `'note' => 'Anticipo / Eccedenza'` (:212 e :224).
<!-- /rettifica -->
...
- **Il documento afferma:** Il filtro della schermata Nuovo Incasso ignora le quote strapagate.
  **Realtà:** Il filtro include oggi anche il caso importo_pagato > importo, con un commento che ne spiega la ragione (evitare il doppio addebito). È uno dei fix della beta.9.
  *Prova:* app/Http/Controllers/Gestionale/Movimenti/SituazioneDebitoriaController.php:26-35 — orWhereRaw('importo_pagato > importo').
