...
- **Spinner di caricamento invisibile sui pulsanti Avanti/Salta/Indietro/Fine:** lo spinner e il testo "Attendere..." erano vincolati con `wire:target` alla sola azione del pulsante stesso (es. `completeStep`), quindi durante un salvataggio "on blur" di un campo (frequente da quando si usa `.live.blur`) il pulsante si disabilitava correttamente ma restava visivamente invariato — su connessioni lente dava l'impressione di un pulsante "bloccato" invece che in caricamento. Rimosso il vincolo `wire:target`: ora spinner e testo reagiscono a qualsiasi richiesta Livewire in corso. Aggiunto anche lo spinner, prima assente, al pulsante "Indietro". *(Nota: non è stato possibile riprodurre in locale la segnalazione "il pulsante Salta non risponde" in tre scenari distinti; questo fix risolve con certezza la mancanza di feedback visivo durante l'attesa, e resta l'ipotesi più plausibile anche per quella segnalazione — da confermare al prossimo test su hosting reale.)*
