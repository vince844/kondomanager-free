...
> **Stato:** Non implementato — verificato il 31/07/2026 su 1.10.0-beta.32, **riverificato riga per riga il 04/08/2026 su 1.10.0-beta.41** prima di aprire la beta.42.
> Nessuna riga di codice è stata scritta: `data_prima_scadenza` non è fra le colonne di `piani_rate` (verificato a database), `modalita_calendario` e la modifica post-generazione non esistono. Il documento resta una specifica valida, non una descrizione del prodotto.
> **Cosa regge, dopo la riverifica:** la partenza cablata su `gestione.data_inizio` è confermata nei tre punti indicati (`GenerateDateRateAction:25` e `:57`, `PianoRateCreatorService:47`); il `?? 1` su `giorno_scadenza` è a `PianoRateCreatorService:38` e il DB dichiara davvero `default('5')`; il check duplicati eventi chiude su `start_time` a `SyncScadenziarioWithPianoRate:148`, accanto a un `rata_id` che da solo basterebbe.
> **Cosa NON regge:** l'affermazione «`rate_quote.data_scadenza` è scritta e mai letta» del §0 punto ③ — vedi il riquadro lì.
> Le due bonifiche del §0 sono ancora aperte e valgono a prescindere dallo spec.
<!-- /verifica-documentazione -->
...
| `?? 1` → rimuovere la chiave (il DB dichiara `default(5)`) | [`PianoRateCreatorService.php:38`](../app/Services/PianoRateCreatorService.php:38) | D1 — oggi il piano è persistito **e generato** al giorno 1, invece che al 5 previsto dallo schema *(diagnosi corretta il 03/08/2026, vedi §D1)* |
...
> ⚠️ **«Scritta e mai letta» è FALSO — corretto il 04/08/2026, riverificato riga per riga su
> 1.10.0-beta.41.** Quella colonna è letta, ed è letta nel punto peggiore in cui potesse
> esserlo: [`SituazioneDebitoriaController`](../app/Http/Controllers/Gestionale/Movimenti/SituazioneDebitoriaController.php)
> ci **ordina** la query delle quote (`:49`), la usa per **risolvere l'esercizio** di
> competenza (`:120-124`) e la mostra all'utente come **scadenza** e come stato
> **«scaduta»** (`:214`, `:225`). È la fonte della scadenza nella schermata di incasso.
>
> Cambia la conseguenza, non la decisione: la sincronizzazione non è un prerequisito
> *architetturale* della Fase 3, è un prerequisito *funzionale* già oggi. Dal primo giorno in
> cui una data si sposta senza essere propagata, la schermata di incasso mostra la scadenza
> vecchia — a chi sta incassando. Non è rimandabile alla 1.10.1.
>
> *(Nota di percorso: il file era stato indicato in `docs/` sotto `PianiRate/`; sta invece in
> `Movimenti/`.)*
...
quel campo viene persistito a `1`. Non c'è una scelta di prodotto
...
> ⚠️ **Diagnosi corretta il 03/08/2026 — questo paragrafo diceva «persistito a `1` e poi
> generato al `5`», e non è vero.** La colonna è `integer('giorno_scadenza')->default(5)`,
> cioè **NOT NULL**: un piano persistito a `1` ha `1` in colonna, quindi il `?? 5` di
> [`GenerateDateRateAction:40`](../app/Actions/PianoRate/GenerateDateRateAction.php:40)
> e `:58` **non scatta mai** e la generazione avviene al giorno `1`. Non c'è nessuna
> divergenza fra persistito e generato: il difetto è che il piano finisce al giorno 1
> invece che al 5 previsto dallo schema. La fix indicata resta identica — cambia il
> sintomo da cercare quando la si verifica.
>
> Vale la pena dirlo perché è costato: nell'analisi del 03/08/2026 questa frase è stata
> ripresa da un agente come se fosse un fatto verificato, ed è finita in un rapporto di
> pianificazione. Un documento che sbaglia la diagnosi è più caro di un documento che
> non c'è.
...
<!-- rettifica -->
> ⚠️ **Non è più vero — verificato il 31/07/2026 su 1.10.0-beta.32.** I riferimenti sono slittati: la rotta è a routes/gestionale.php:182 e la validazione a PianoRateController.php:644 (il controller è stato modificato in beta.32). Il contenuto delle due affermazioni è corretto, sbagliato è solo il numero di riga.
> *Prova:* routes/gestionale.php:182 (`->name('piani-rate.update-stato')`); app/Http/Controllers/Gestionale/PianiRate/PianoRateController.php:644 (`'data_delibera_assemblea' => 'required_if:approvato,true|nullable|date'`)
<!-- /rettifica -->
...
In più, chiude un difetto di documentazione: [`guida_preventivi_rate_capitoli.md:95`](guida_preventivi_rate_capitoli.md) — la stessa riga era anche in `logica_piani_rate.md`, archiviato il 16/08/2026 perché ne era una copia più vecchia
...
<!-- rettifica -->
> ⚠️ **Non è più vero — verificato il 31/07/2026 su 1.10.0-beta.32.** Entrambe le guide dicono "giorno di scadenza", non "data prima scadenza", e portano già una nota datata 2026-07-29 che spiega che il campo è solo pianificato. Il "difetto di documentazione" che il paragrafo si propone di chiudere è già stato chiuso.
> *Prova:* docs/guida_preventivi_rate_capitoli.md:95 ("giorno di scadenza") e le note a docs/guida_preventivi_rate_capitoli.md:98-101. La stessa prova stava in `logica_piani_rate.md`, ora in `docs/archivio/`
<!-- /rettifica -->
...
La collocazione in 1.10.1 resta valida: la 1.10.0 è già molto carica, e infilarci una
feature che tocca scadenzario ed eventi condòmino significa rimettere in gioco
superfici già stabilizzate.

> ⚠️ **Aggiornato il 03/08/2026.** Questo paragrafo motivava la collocazione con
> «la 1.10.1 è oggi **Export SEPA**, quindi le due funzionalità si rinforzano».
> **La premessa non vale più:** l'Export SEPA è stato spostato in **v1.16** il
> 31/07/2026 (decisione vigente in [`roadmap.md`](roadmap.md), «Decisione di scope»),
> perché `Pain.001` è ciò che la banca ingerisce e la riconciliazione bancaria — cuore
> della 1.16 — è ciò che la banca restituisce: stessa superficie, stesso collaudo
> multi-banca. La 1.10.1 è oggi **Calendario rate Fasi 2-3 + Importatore dati**.
> Anche il riferimento a «beta.29» è superato: siamo alla `beta.38`.
>
> Chi usasse il paragrafo originale per decidere una collocazione, deciderebbe su una
> premessa morta.
