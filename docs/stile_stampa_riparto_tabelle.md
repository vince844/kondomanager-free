...
> Formato pagina, chunking a 8 tabelle, palette e convenzioni grafiche sono corretti; NON fidarti della tabella delle larghezze (la colonna App. è 7%, non 3,5%, quindi il residuo per le tabelle è 50,5%/54,5%) né della nota sul font adattivo, che dipende dal totale delle tabelle e non dal blocco corrente.
<!-- /verifica-documentazione -->
...
<!-- rettifica -->
> ⚠️ **Non è più vero — verificato il 31/07/2026 su 1.10.0-beta.32.** I font sono calcolati una sola volta sul numero TOTALE di tabelle del piano, fuori dal loop dei chunk: un piano da 10 tabelle spezzato in 8+2 stampa anche il blocco da 2 tabelle a 6pt/5pt/4.5pt, non a 7pt. Solo le larghezze sono per blocco.
> *Prova:* resources/views/pdf/gestionale/riparto_tabelle.blade.php:14 `$nTab = count($tabelle);` e :18-20 $fontBase/$fontSmall/$fontTiny da $nTab, entrambi PRIMA del `@foreach($chunksTabelle ...)` di riga 146; dentro il loop si ricalcolano solo $wPerTab/$wQuota/$wImporto (righe 148-151)
<!-- /rettifica -->
OGNI blocco.
...
- **Il documento afferma:** La colonna "App." occupa il 3,5% della pagina, ristretta da 5 a 3,5 con il feedback Gadotti.
  **Realtà:** La colonna App. è larga 7%, il doppio di quanto dichiarato. Il commit 83f0bf60 (beta.15) l'ha allargata per ospitare nome unità + interno + piano, e il documento non è stato aggiornato.
  *Prova:* resources/views/pdf/gestionale/riparto_tabelle.blade.php:26 `$wApp    = 7;` (le altre colonne combaciano: $wNome 18/22 riga 27, $wRuolo 3 riga 28, $wTotSogg 7 riga 29, $wPct 3.5 riga 30, $wTotApt 7 riga 31); git log -S'$wApp    = 7' → 83f0bf60

- **Il documento afferma:** Alle tabelle millesimali resta il 54% della pagina con ≤5 tabelle e il 58% con >5.
  **Realtà:** Il residuo reale è 50,5% e 54,5%: 3,5 punti in meno in entrambi i casi, conseguenza della colonna App. a 7%. Chi dimensiona una nuova colonna partendo da questa tabella sfora.
  *Prova:* resources/views/pdf/gestionale/riparto_tabelle.blade.php:32-33 `$wFixed = $wApp + $wNome + $wRuolo + $wTotSogg + $wPct + $wTotApt; $wTabCols = 100 - $wFixed;` con $wApp=7 → 49,5 e 45,5 di fisse

- **Il documento afferma:** Fra un immobile e l'altro c'è un separatore orizzontale da 3px.
  **Realtà:** Non esiste alcun bordo orizzontale da 3px: il 3px è un accento VERTICALE navy sul lato sinistro della colonna App. Gli unici border-top spessi (2px) sono nella riga dei totali.
  *Prova:* resources/views/pdf/gestionale/riparto_tabelle.blade.php:276 `border-left: 3px solid {{ $navy }};`; grep "border-top" sullo stesso file → solo righe 389-422 (2px, tfoot) e 457 (1px, nota legale)
