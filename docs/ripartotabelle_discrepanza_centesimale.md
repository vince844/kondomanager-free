...
> Leggere solo l'appendice "Risoluzione definitiva", che corrisponde al codice attuale; il corpo del documento descrive fix superati (l'`usort` della Fase 1 non è più nel service) e rimanda a un PDF di riferimento che nel repository non esiste. **Potato il 16/08/2026 su 1.10.0-beta.55:** l'avviso sta ora anche in testa a «Fix Pianificato — v1.10» e a «Documento di Riferimento T12», così chi atterra a metà documento lo vede senza tornare qui.
<!-- /verifica-documentazione -->
...
<!-- rettifica -->
> ⚠️ **Non è più vero — verificato il 31/07/2026 su 1.10.0-beta.32.** La directory `tests/fixtures/` non esiste affatto nel repository, quindi nessun test e nessun lettore può risalire a quei valori.
> *Prova:* `ls tests/fixtures/` → "No such file or directory"; grep "fixtures" su tests/*.php = 0 risultati
<!-- /rettifica -->
---
## Contesto
...
<!-- rettifica -->
> ⛔ **Tutto ciò che segue fino all'appendice è superato — potatura del 16/08/2026.** Nessuna delle
> due fasi è il codice di oggi: l'`usort` della Fase 1 **non è più** nel service, e la Fase 2 è
> stata sostituita da una variante più semplice, descritta nell'**appendice «Risoluzione
> definitiva»** in fondo al documento. Vai direttamente lì.
>
> Queste pagine restano perché raccontano **due strade scartate e il perché**, che è l'unico posto
> in cui quel ragionamento è scritto — ma come ricette sono da non seguire.
<!-- /rettifica -->
...
<!-- rettifica -->
> ⚠️ **Non è più vero — verificato il 31/07/2026 su 1.10.0-beta.32.** In `RipartoTabelleService.php` non esiste alcuna `usort`: il fix Fase 1 è stato rimosso con la riscrittura conto-per-conto (l'appendice lo dichiara "superato", ma il corpo del documento resta al presente e induce in errore).
> *Prova:* grep -c "usort" app/Services/RipartoTabelleService.php = 0; `buildMatrice()` è a app/Services/RipartoTabelleService.php:70
<!-- /rettifica -->
...
<!-- rettifica -->
> ⚠️ **Il PDF citato qui non è nel repository**, e non c'è modo di ritrovarlo dai riferimenti di
> questa sezione: i numeri riportati restano leggibili, la fonte no.
<!-- /rettifica -->
