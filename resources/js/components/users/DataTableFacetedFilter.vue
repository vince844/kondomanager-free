<!--
  Filtro a scelta multipla per l'elenco utenti.

  Copiato dal gemello di `segnalazioni/` — che è la convenzione del progetto, dove ogni modulo ha
  il suo — e allineato alle chiavi di lingua `users.table.*`. ⚠️ Le quattro copie esistenti sono
  già **divergenti** fra loro: questa è la quinta, e la loro unificazione è un lavoro a sé.
-->
      ...
      <Button variant="outline" size="sm" class="h-8 border-dashed">
        ...
        <template v-if="selectedValues.size > 0">
          ...
          <div class="hidden space-x-1 lg:flex">
            ...
            >
              {{ selectedValues.size }} {{ trans('users.table.selected') }}
            </Badge>
            <template v-else>
              <Badge
    ...
    <PopoverContent class="w-[250px] p-0" align="start">
      <Command>
        ...
        <CommandList v-if="props.isLoading">
          <div class="p-4 text-sm text-muted-foreground">{{trans('users.table.loading')}}</div>
        </CommandList>

        <CommandList>
          <CommandEmpty>{{trans('users.table.no_results')}}</CommandEmpty>
          <CommandGroup>
            <CommandItem
          ...
          <template v-if="selectedValues.size > 0">
            ...
            <CommandGroup>
              <CommandItem
                :value="{ label: trans('users.table.clear_all_filters') }"
