...
import DataTableToolbar from '@/components/gestionale/movimenti/giroconti/DataTableToolbar.vue';
...
const props = defineProps<{
  columns: ColumnDef<GirocontoRow>[],
  data: GirocontoRow[],
  condominio: Building,
  meta: {
    current_page: number,
...
const table = useVueTable({
  ...
  onPaginationChange: updater => {
    ...
    // La ricerca attiva viaggia con la pagina: senza, cambiare pagina
    // azzererebbe silenziosamente il filtro.
    const searchAttiva = new URLSearchParams(window.location.search).get('search');

    router.get(route(generateRoute('gestionale.giroconti.index'), { condominio: props.condominio.id }), {
      page: nextPage + 1,
      per_page: nextPageSize,
      ...(searchAttiva ? { search: searchAttiva } : {}),
    }, {
      preserveState: true,
      preserveScroll: true,
      replace: true,
      ...
      <Empty v-else class="py-12 bg-slate-50/50">
        <EmptyHeader class="max-w-4xl">
          <EmptyMedia variant="icon" class="bg-violet-50/50 dark:bg-violet-900/20 text-violet-500">
            <Repeat2 class="w-8 h-8" />
          </EmptyMedia>
          <EmptyTitle>Nessun giroconto registrato</EmptyTitle>
          <EmptyDescription>
            Non ci sono spostamenti fra casse che corrispondono ai criteri. <br>
            Registra un nuovo giroconto o modifica i filtri di ricerca.
          </EmptyDescription>
        </EmptyHeader>
      </Empty>
