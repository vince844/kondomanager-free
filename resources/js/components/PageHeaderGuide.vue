  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      ...
      >
        <template v-for="(item, index) in breadcrumbs" :key="index">
          <Link
            ...
            :href="item.href"
            class="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500 hover:text-primary transition-colors"
          >
            {{ item.title }}
          </Link>
      ...
      <div class="flex flex-wrap items-center gap-2 shrink-0">
          ...
          <DropdownMenuPortal>
            <DropdownMenuContent align="end" class="min-w-[180px]">
              <DropdownMenuItem
                ...
                :class="{ 'font-semibold text-primary': c.id === condominio.id }"
        ...
        <div
          v-else
          class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm text-xs font-medium"
        >
          <Building2 class="w-3.5 h-3.5 text-slate-400" />
            ...
            <DropdownMenuPortal>
              <DropdownMenuContent align="end" class="min-w-[180px]">
                <DropdownMenuItem
                  ...
                  :key="e.id"
                  class="cursor-pointer"
                  :class="{ 'font-semibold text-primary': e.id === esercizio?.id }"
                  @click="selectEsercizio(e.id)"
                >
                  {{ e.nome }}
                </DropdownMenuItem>
              </DropdownMenuContent>
        ...
        <a
          ...
          :href="videoUrl"
          target="_blank"
          class="inline-flex items-center gap-2 text-xs font-bold px-3 py-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 border border-red-100 transition-colors dark:bg-red-950/30 dark:text-red-400 dark:border-red-900/30"
        >
    ...
    <div class="pl-5 border-l-1 border-primary/40 dark:border-primary/40 py-1">
    ...
    >
      <div
        ...
        :class="{ 'md:px-6 md:border-l border-slate-200 dark:border-slate-800': index > 0 }"
      >
