  ...
  <AppLayout>
    ...
    <FornitoreLayout>
      ...
      <PageHeaderGuide
        ...
        back-text="Indietro"
      >
        <!-- Breadcrumb sostituito dalle tab stato -->
        <template #breadcrumb>
          <div class="flex items-center p-0.5 bg-slate-100 dark:bg-slate-900/60 rounded-lg border border-slate-200 dark:border-slate-800">
            <span
              v-for="s in stati"
              :key="s.value"
              class="px-3.5 py-1 text-[11px] font-bold uppercase tracking-widest rounded-md transition-all duration-200 select-none"
        ...
        <!-- Pulsante Modifica nello slot actions -->
        <template #actions>
          <Link
            as="button"
            :href="generatePath('fornitori/:fornitore/edit', { fornitore: props.fornitore.id })"
            class="inline-flex items-center gap-2 rounded-lg bg-slate-800 dark:bg-slate-200 px-4 py-2 text-sm font-medium text-white dark:text-slate-900 hover:bg-slate-700 dark:hover:bg-white transition-colors shadow-sm"
          >
      ...
      <!-- Grid principale -->
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-16 mt-6">
          ...
          <div class="rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/80 dark:border-slate-700/50">
            ...
            <div class="grid grid-cols-2 divide-x divide-y divide-slate-200/60 dark:divide-slate-700/40">
              <div class="px-5 py-4">
                ...
                <span class="font-mono text-sm text-slate-800 dark:text-slate-200">{{ fornitore.codice_fiscale || '—' }}</span>
              </div>
              <div class="px-5 py-4">
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-1">Codice ATECO</span>
                <span class="font-mono text-sm text-slate-800 dark:text-slate-200">{{ fornitore.codice_ateco || '—' }}</span>
              </div>
              <div class="px-5 py-4">
        ...
        <div class="space-y-6">
          ...
          <div class="rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/80 dark:border-slate-700/50">
            ...
            <div class="px-5 py-4 border-b border-slate-200/60 dark:border-slate-700/40">
              ...
              <span v-if="fornitore.iban_principale" class="font-mono text-sm text-slate-800 dark:text-slate-200 break-all leading-relaxed block">
            ...
            <div v-if="fornitore.soggetto_ritenuta" class="px-5 py-4">
              ...
              <div class="grid grid-cols-3 gap-2">
                <div class="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20 rounded-lg px-2 py-2.5 text-center">
                  ...
                  <span class="font-mono font-bold text-sm text-amber-900 dark:text-amber-400">{{ fornitore.perc_ritenuta || '0' }}%</span>
                </div>
                <div class="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20 rounded-lg px-2 py-2.5 text-center">
                  <span class="text-[9px] font-bold uppercase tracking-wider text-amber-600/70 dark:text-amber-500/60 block mb-1">Impon.</span>
                  <span class="font-mono font-bold text-sm text-amber-900 dark:text-amber-400">{{ fornitore.perc_imponibile_ritenuta || '100' }}%</span>
                </div>
                <div class="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20 rounded-lg px-2 py-2.5 text-center">
                  <span class="text-[9px] font-bold uppercase tracking-wider text-amber-600/70 dark:text-amber-500/60 block mb-1">Tributo</span>
                  <span class="font-mono font-bold text-sm text-amber-900 dark:text-amber-400">{{ fornitore.codice_tributo || '—' }}</span>
                </div>
              </div>
            </div>
