...
// Filtro rapido sulle card
const filterSfori = () => {
    if (props.stats.totale_sfori > 0) {
        router.get(
            route(generateRoute('gestionale.fatture.index'), { condominio: props.condominio.id }),
            { stato_approvazione: 'sforo_motivato' },
            { preserveState: true }
        );
    } else {
        // Reset filtro se clicco su zero
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                            ...
                            <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-2xl font-black text-slate-900">{{ formatEuro(stats.importo_da_pagare) }}</p>
                                </div>
                            </div>
                            <div @click="filterSfori" class="border rounded-xl p-4 flex items-center gap-4 transition-all"
                                :class="stats.totale_sfori > 0 || filters.stato_approvazione === 'sforo_motivato' ? 'bg-orange-50 border-orange-200 hover:bg-orange-100 cursor-pointer shadow-sm' : 'bg-white border-slate-200 opacity-70'">
                                <div :class="stats.totale_sfori > 0 || filters.stato_approvazione === 'sforo_motivato' ? 'bg-orange-100 border border-orange-200' : 'bg-slate-100'" class="p-2.5 rounded-lg">
                                    <AlertTriangle :class="stats.totale_sfori > 0 || filters.stato_approvazione === 'sforo_motivato' ? 'text-orange-600' : 'text-slate-400'" class="w-5 h-5" />
                                </div>
                                <div>
                                    <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Da Ratificare</p>
                                    <p class="text-2xl font-black" :class="stats.totale_sfori > 0 || filters.stato_approvazione === 'sforo_motivato' ? 'text-orange-700' : 'text-slate-900'">
                                        {{ stats.totale_sfori }}
                                    </p>
                                </div>
                            </div>
