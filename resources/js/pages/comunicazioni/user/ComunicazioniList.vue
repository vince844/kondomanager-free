...
const comunicazioneID = ref<string>('');
...
const filteredResults = computed(() => {
  if (!searchQuery.value) return comunicazioni.value;
  const query = searchQuery.value.toLowerCase();
  return comunicazioni.value.filter(c =>
...
async function confirmDelete() {
  ...
  const comunicazioneToDelete = comunicazioni.value.find(c => c.id === id);
