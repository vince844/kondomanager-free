...
<template>
    <Head :title="trans('auth.header.login.head')" />

    <AuthBase 
        :title="trans('auth.header.login.title')" 
        :description="trans('auth.header.login.description')"
    >
            ...
            <div class="grid gap-6">
                ...
                <div class="grid gap-2">
                    <div class="flex items-center justify-between">
                        <Label for="password">{{ trans('auth.label.login.password') }}</Label>
                        <TextLink v-if="canResetPassword" :href="route('password.request')" class="text-sm" :tabindex="5">
                            {{ trans('auth.link.forgot_password') }}
                        </TextLink>
                    </div>
                    <Input
                        id="password"
                        ...
                        :placeholder="trans('auth.placeholder.login.password')"
                    />
                ...
                <div class="flex items-center justify-between" :tabindex="3">
                    <Label for="remember" class="flex items-center space-x-3">
                        ...
                        <span>{{ trans('auth.label.login.remember') }}</span>
                    </Label>
                </div>

                <Button type="submit" class="mt-4 w-full" :tabindex="4" :disabled="form.processing">
                    <LoaderCircle v-if="form.processing" class="h-4 w-4 animate-spin" />
                    {{ trans('auth.button.login') }}
                </Button>
            </div>
